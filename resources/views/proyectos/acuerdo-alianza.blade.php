@php
    ...
    /** @var \Illuminate\Support\Collection<int,\App\Models\ProjectPartner> $partes */
...
<head>
    ...
    <title>Acuerdo de alianza · {{ $proyecto->code }} · {{ $v['laboratorio'] }}</title>
    <style>
...
<h2>Las partes</h2>
<table class="partes">
    <tr><td class="k">El proyecto</td><td>«{{ $v['proyecto'] }}» ({{ $v['codigo'] }}), en {{ $v['laboratorio'] }}, de {{ $v['institucion'] }}. Responde por el proyecto: {{ $v['responsable'] }}.</td></tr>
    @foreach ($partes as $p)
        <tr>
            <td class="k">{{ $p->papel() }}</td>
            <td>
                {{ $p->quien() }}@if ($p->document) · {{ $p->document }}@endif
                <br><span style="color:#666">Pone: {{ $p->aporteLegible() }}@if ($p->share_percent !== null) · participación {{ rtrim(rtrim(number_format((float) $p->share_percent, 2, ',', '.'), '0'), ',') }} %@endif</span>
            </td>
        </tr>
    @endforeach
    <tr><td class="k">Total de aportes</td><td>{{ $v['total'] }}</td></tr>
</table>
...
<table class="firmas">
    @foreach ($partes->chunk(2) as $fila)
        <tr>
            @foreach ($fila as $p)
                <td>
                    <div class="linea">
                        {{ $p->esElLaboratorio() ? 'Por el laboratorio' : 'Por ' . $p->quien() }}
                        <small>{{ $p->esElLaboratorio() ? $v['responsable'] . ' · ' . $v['laboratorio'] : mb_strtolower($p->papel()) . ($p->document ? ' · ' . $p->document : '') }}</small>
                    </div>
                </td>
            @endforeach
            @if ($fila->count() === 1)<td></td>@endif
        </tr>
    @endforeach
</table>
